...
def discriminator(x, drop_out):
    ...
    h1 = tf.nn.leaky_relu(tf.matmul(h0, w1) + b1)
    h11 = tf.nn.dropout(h1, 0.3)
    #3rd hidden layer
    w2 = tf.get_variable('D_w2', [h11.get_shape()[1], 512], initializer=w_init)
    b2 = tf.get_variable('D_b2', [512], initializer=b_init)
    h2 = tf.nn.leaky_relu(tf.matmul(h11, w2) + b2)
    h22 = tf.nn.dropout(h2, 0.3)
    #4th hidden layer
    w3 = tf.get_variable('D_w3', [h22.get_shape()[1], 256], initializer=w_init)
    b3 = tf.get_variable('D_b3', [256], initializer=b_init)
    h3 = tf.nn.leaky_relu(tf.matmul(h22, w3) + b3)
    h33 = tf.nn.dropout(h3, 0.3)
    # output layer
    w4 = tf.get_variable('D_w4', [h33.get_shape()[1], 1], initializer=w_init)
    b4 = tf.get_variable('D_b4', [1], initializer=b_init)
    o4 = tf.sigmoid(tf.matmul(h33, w4) + b4)
    return o4
...
def show_result(num_epoch, show = True, save = True, path = 'result.png',isFix=False):
    ...
    if isFix:
        test = sess.run(G_z, {z: fixed_z_, drop_out: 0.0})
    else:
        test = sess.run(G_z, {z: z_, drop_out: 0.0})

    size_1 = 5
    figure, axis = plt.subplots(size_1, size_1, figsize=(5, 5))
    for i, j in itertools.product(range(size_1), range(size_1)):
    ...
    for k in range(5*5):
        ...
        j = k % 5
        axis[i, j].cla()
        axis[i, j].imshow(np.reshape(test[k], (28, 28)), cmap='gray')
...
for epoch in range(train_epoch):
    ...
    p = 'C:/Users/user/Desktop/queen-mary/Deep_Learning/CW2/MNIST_GAN_' + str(epoch + 1) + '.png'
    fixed_p = 'C:/Users/user/Desktop/queen-mary/Deep_Learning/CW2/MNIST_GAN_' + str(epoch + 1) + '.png'
...
show_train_hist(train_hist, save=True, path='C:/Users/user/Desktop/queen-mary/Deep_Learning/CW2/MNIST_GAN_train_hist.png')
images = []
sess.close()